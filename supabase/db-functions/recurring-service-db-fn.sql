...
AS $function$
  ...
  begin
    ...
    -- for each recurring
    FOR r in SELECT id, frequency, next_at, amount, sub_category_id, category_id, budget_id, EXTRACT('year' FROM next_at AT TIME ZONE time_zone) AS year, EXTRACT('month' FROM next_at AT TIME ZONE time_zone) AS month, EXTRACT('week' FROM next_at AT TIME ZONE time_zone) AS week, EXTRACT(ISODOW FROM next_at AT TIME ZONE time_zone) AS day  from recurring
      WHERE 
        DATE_TRUNC('day', recurring.next_at AT TIME ZONE time_zone) = DATE_TRUNC('day', NOW() AT TIME ZONE time_zone) and active = TRUE
    LOOP
